...
const log = createLogger('Admin/PerformersListPage');
...
export default function PerformersListPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await performerService.listAdmin(query || undefined, page, pageSize);
      setItems(data.items);
  ...
  const onDelete = async (p: Performer) => {
    try {
      await performerService.remove(p.id);
      message.success('Performer deleted');
      await load();
    } catch (err: unknown) {
      const e = err as { response?: { status?: number; data?: { message?: string } } };
      if (e?.response?.status === 409) {
        message.error(e.response.data?.message ?? 'Performer is linked to events');
      } else {
  ...
  return (
    <PageShell
      title="Performers"
      subtitle="People and acts that appear on your events"
      extra={
        <Button
          type="primary"
          icon={<PlusOutlined />}
          onClick={() => navigate('/performers/new')}
    ...
    >
      ...
      <LoadingBoundary
        ...
        skeleton="card"
        empty={{
          title: query ? 'No performers match your search' : 'No performers yet',
          description: query ? 'Try a different search term.' : 'Add your first performer to attach to events.',
          actionLabel: query ? undefined : 'Add First Performer',
          onAction: query ? undefined : () => navigate('/performers/new'),
        }}
      >
        {(list) => (
          <>
                  ...
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 16 }}>
                    ...
                    >
                      {p.primaryImageUrl ? (
                        ...
                        <UserOutlined style={{ fontSize: 28, color: 'var(--text-muted)' }} />
                      )}
                    </div>
