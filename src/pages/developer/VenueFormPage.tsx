...
export default function VenueFormPage(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    async function fetchVenue(): Promise<void> {
      try {
        const res = await apiClient.get<VenueData>(`/developer/venues/${id}`);
        if (cancelled) return;
        const v = res.data;
        reset({
          name: v.name ?? '',
  ...
  async function onSubmit(values: VenueFormValues): Promise<void> {
    ...
    try {
      if (isEdit && id) {
        await apiClient.put(`/developer/venues/${id}`, values);
        toast.success('Venue updated successfully');
      } else {
        await apiClient.post('/developer/venues', values);
        toast.success('Venue created successfully');
      }
      navigate('/developer/venues');
    } catch {
      ...
      {fetchLoading ? (
        ...
        >
          ...
          >
            <button
              type="button"
              onClick={() => navigate('/developer/venues')}
              disabled={submitting}
              style={{
