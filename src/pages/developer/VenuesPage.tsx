...
export default function VenuesPage(): React.ReactElement {
  ...
  const fetchVenues = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const res = await apiClient.get<PaginatedResponse>('/developer/venues', { params });
      setData(res.data);
    } catch {
  ...
  async function handleToggleVenueActive(venueId: string, active: boolean): Promise<void> {
    try {
      if (active) {
        await apiClient.put(`/developer/venues/${venueId}`, { isActive: true });
      } else {
        await apiClient.delete(`/developer/venues/${venueId}`);
      }
  ...
  async function handleDelete(): Promise<void> {
    ...
    setDeleting(true);
    try {
      await apiClient.delete(`/developer/venues/${deleteTarget.id}`);
                    ...
                    >
                      ...
                      <td style={{ padding: '0.875rem 1rem', textAlign: 'center', whiteSpace: 'nowrap' }}>
                        <div style={{ display: 'inline-flex', gap: '0.375rem', alignItems: 'center' }}>
                          {venue.isActive ? (
                            <Link
                              to={`/developer/venues/${venue.id}/edit`}
