...
function NoUpcomingEvents({ basePath }: { basePath: string }): React.ReactElement {
...
export default function DashboardPage(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    async function fetch(): Promise<void> {
      try {
        const res = await api.dashboard.getNextEvent<ApiResponse>();
        if (cancelled) return;
  ...
  if (!hasUpcoming || !data) return <NoUpcomingEvents basePath={basePath} />;
  ...
  // Admin can deep-link to individual event pages; developer navigates to event list
  const eventDetailPath = role === 'admin'
    ? `${basePath}/events/${d.eventId}`
    : `${basePath}/events`;
  const bookingsPath = role === 'admin'
    ? `${basePath}/events/${d.eventId}?tab=bookings`
    : `${basePath}/events`;
