...
const wizardSchema = z
  .object({
    ...
    layoutMode: z.enum(['Grid', 'CapacityOnly', 'None'], { message: 'Layout mode is required' }),
...
function NewVenueModal({
  ...
  async function onSubmit(values: NewVenueFormValues): Promise<void> {
    ...
    try {
      ...
      const res = await api.venues.create<Venue>(payload);
...
interface Step2LayoutPanelProps {
  ...
  GridEditorComponent: React.LazyExoticComponent<React.ComponentType<{ eventId: string }>>;
}
        ...
        <div style={{ marginTop: '0.5rem' }}>
          ...
          {(isEdit && eventId) ? (
            <Suspense fallback={
              ...
              <GridEditorComponent eventId={eventId} />
            </Suspense>
          ) : (
            <div
              style={{
...
export default function EventWizardPage(): React.ReactElement {
  ...
  // Lazy-load role-specific editors
  const GridEditorComponent = useMemo(
    () => lazy(() =>
      role === 'admin'
        ? import('../admin/editors/GridEditor')
        : import('../developer/editors/GridEditor')
    ),
    [role],
  );

  const PricingStepComponent = useMemo(
    () => lazy(() =>
      role === 'admin'
        ? import('../admin/editors/PricingStep')
        : import('../developer/editors/PricingStep')
    ),
    [role],
  );

  const ReviewStepComponent = useMemo(
    () => lazy(() => import('./editors/ReviewStep')),
    [],
  );
  ...
  useEffect(() => {
    ...
    async function loadVenues(): Promise<void> {
      try {
        const res = await api.venues.list<VenueListResponse>({ pageSize: 100 });
  ...
  useEffect(() => {
    ...
    async function loadEvent(): Promise<void> {
      try {
        const res = await api.events.getById<EventApiObject>(id!);
        if (cancelled) return;
        const e = res.data;
    ...
    void loadEvent();
    return () => { cancelled = true; };
  }, [id, isEdit, api, resetForm, setFormData, setEditingEvent]);

  // Check if layout mode is locked (admin-only: has bookings or holds)
  useEffect(() => {
    if (!isEdit || !id) return;
    if (!('checkLayoutLocked' in api.events)) return;
    let cancelled = false;
    async function checkLocked(): Promise<void> {
      try {
        const checkFn = (api.events as { checkLayoutLocked: (id: string) => Promise<{ data: { locked: boolean } }> }).checkLayoutLocked;
        const res = await checkFn(id!);
  ...
  async function handleNext(): Promise<void> {
    ...
    if (step === 1) {
      ...
      try {
        const res = await api.pricing.list<Array<{ id: string; isActive: boolean; type: string }>>(id);
