...
export default function ReviewStep({
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      try {
        const res = await api.events.getById<{
          status: string;
          venueId: string;
          venue?: Venue;
        }>(eventId!);
        ...
        if (res.data.venue) {
          ...
          try {
            const vRes = await api.venues.getById<Venue>(res.data.venueId);
  ...
  useEffect(() => {
    ...
    async function loadLayout(): Promise<void> {
      try {
        const res = await api.layout.get<LayoutData>(eventId!);
    ...
    void loadLayout();
    return () => { cancelled = true; };
  }, [eventId, formData.layoutMode, api]);
    ...
    async function loadPricing(): Promise<void> {
      try {
        const [rulesRes, resolvedRes] = await Promise.all([
          api.pricing.list<PricingRule[]>(eventId!),
          api.pricing.resolve<ResolvedPrice>(eventId!).catch(() => null),
        ]);
        if (!cancelled) {
  ...
  async function handlePublish(): Promise<void> {
    ...
    try {
      ...
      if (!isAlreadyPublished) {
        await api.events.updateStatus(eventId, 'Published');
