...
export default function PricingStep({ eventId, layoutMode, maxCapacity }: PricingStepProps): React.ReactElement {
  ...
  // Platform fee state (developer only)
  ...
  const loadRules = useCallback(async (): Promise<void> => {
    ...
    try {
      const res = await api.pricing.list<PricingRule[]>(eventId);
  ...
  useEffect(() => {
    ...
    async function resolvePrice(): Promise<void> {
      ...
      try {
        const res = await api.pricing.resolve<ResolvedPrice>(eventId!);
  ...
  async function handleAddRule(values: RuleFormValues): Promise<void> {
    ...
    try {
      ...
      await api.pricing.create(eventId, payload);
  ...
  async function handleEditRule(ruleId: string, values: RuleFormValues): Promise<void> {
    ...
    try {
      ...
      await api.pricing.update(eventId, ruleId, payload);
  ...
  async function handleToggleActive(rule: PricingRule): Promise<void> {
    if (!eventId) return;
    try {
      await api.pricing.update(eventId, rule.id, {
  ...
  async function handleDelete(): Promise<void> {
    ...
    setDeleting(true);
    try {
      await api.pricing.delete(eventId, deleteTarget.id);
  ...
  async function handleDrop(dropIndex: number): Promise<void> {
    ...
    if (!eventId) return;
    try {
      await api.pricing.reorder(eventId, {
  ...
  // Platform fee preview (developer only)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {layoutMode === 'Grid' && (
        <div style={sectionCard}>
          ...
          <div style={{ marginTop: '0.875rem' }}>
            <a
              href={`${basePath}/events/${eventId}/layout`}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: '0.375rem',
      ...
      {/* ── Section 4: Platform Fee (developer only) ────────────────────── */}
      {role === 'developer' && (
        <div style={sectionCard}>
