...
function TableCell({ element, isSelected, isBooked, bookedLabel, bookedTitle, onSelect, onContextMenu, onDragStart }: TableCellProps): React.ReactElement {
  ...
  return (
    <div
      ...
      title={isBooked ? `${element.label} — ${bookedTitle}` : element.label}
      style={{
        width: '100%',
        height: '100%',
        display: 'flex',
...
export default function GridEditor({ eventId }: GridEditorProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      try {
        const res = await api.tableTypes.list<TableType[]>();
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      ...
      try {
        ...
        const draftRes = await api.layout.getDraft<DraftResponse>(eventId!);
        ...
        // Fall back to DB
        const res = await api.layout.get<ApiLayoutResponse>(eventId!);
        if (!cancelled) {
  ...
  // Fetch locked table IDs and revenue stats
  useEffect(() => {
    if (!eventId) return;

    if (role === 'admin' && 'getLocked' in api.layout) {
      // Admin: use dedicated getLocked endpoint
      const getLocked = api.layout.getLocked as <T>(id: string) => Promise<{ data: T }>;
      getLocked<{ lockedTableIds: string[] }>(eventId)
        .then((res) => {
      ...
      if ('getStats' in api.bookings) {
        const getStats = api.bookings.getStats as (id: string) => Promise<{ data: unknown }>;
        getStats(eventId)
          .then((res) => {
      ...
      // Developer: derive locked tables from booking data
      api.bookings.list(`eventId=${eventId}&pageSize=100`)
        .then((res) => {
          ...
          // Mark tables as booked if there are any paid bookings
          if (paid.length > 0) {
  ...
  function handleDelete(id: string): void {
    if (bookedTableIds.has(id)) {
      toast.error(role === 'admin' ? 'Cannot delete a table with active holds or bookings' : 'Cannot delete a sold table');
      return;
    }
    deleteElement(id);
    clearSelection();
  ...
  async function performAutoSave(): Promise<void> {
    ...
    try {
      await api.layout.saveDraft(eventId, buildLayoutPayload());
      setSaveStatus('saved');
    } catch {
      // Redis draft failed — try direct DB save as fallback
      try {
        await api.layout.save(eventId, buildLayoutPayload());
        markClean();
        setSaveStatus('saved');
      } catch {
        setSaveStatus('idle');
  ...
  async function handleSave(): Promise<void> {
    ...
    try {
      ...
      try {
        await api.layout.saveDraft(eventId, buildLayoutPayload());
        await api.layout.flush(eventId);
      } catch {
                    ...
                    >
                      {occupant ? (
                        <TableCell
                          ...
                          bookedLabel={role === 'admin' ? 'LOCKED' : 'SOLD'}
                          bookedTitle={role === 'admin' ? 'LOCKED (held or booked)' : 'SOLD (locked)'}
